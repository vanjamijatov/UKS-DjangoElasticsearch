...
def brisanje_artikla(request, id):
    ...
    try:
        search_request = Search(using='default', index=ArtikalDokument.Index.name).query('match', id=id)
...
def unos_artikla(request, id=None):
    ...
    if request.method == 'GET':
        ...
        if id is None:
            return render(request, "unos_artikla.html",
                          {"title": title, "prodavnice": prodavnice, "lista_kategorija": lista_kategorija})
        else:
            stari_id = id
            ...
            return render(request, "unos_artikla.html", {"title": title, "stari_id": stari_id, "oznaka": a.oznaka,
                                                         "naziv": a.naziv, "opis": a.opis, "cena": a.cena,
                                                         "na_akciji": a.na_akciji, "kategorije": kategorije,
                                                         "prodavnica": a.prodavnica, "prodavnice": prodavnice,
        ...
        if greska_oznaka is None and greska_naziv is None and greska_opis is None and greska_cena is None \
            ...
            if stari_id is None:
                ...
                for k in kategorije:
                    a.kategorije.add(k)
                a.save()
                ArtikalDokument(meta={'id': str(a.id)}, id=str(a.id), naziv=a.naziv, opis=a.opis,
                                cena=a.cena, prodavnica=a.prodavnica.naziv).save()
            else:
